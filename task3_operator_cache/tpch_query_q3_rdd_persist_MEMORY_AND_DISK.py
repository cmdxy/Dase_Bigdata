...
def main():
    # 初始化 SparkContext
    conf = SparkConf().setAppName("TPCH_Query3_persist_MEMORY_AND_DISK")
    ...
    try:
        ...
        customers.persist(StorageLevel.MEMORY_AND_DISK)  # 使用 persist() 并指定存储级别
        ...
        orders.persist(StorageLevel.MEMORY_AND_DISK)  # 使用 persist() 并指定存储级别
        ...
        lineitems.persist(StorageLevel.MEMORY_AND_DISK)  # 使用 persist() 并指定存储级别
        ...
        # 将结果写入本地文件
        with open('q3_results_persist_MEMORY_AND_DISK.txt', 'w') as f:
            f.write(header)
            for row in all_results:
                f.write(row + '\n')

        print("Results have been saved to q3_results_persist_MEMORY_AND_DISK.txt")
        ...
        # 保存性能指标
        with open('q3_performance_persist_MEMORY_AND_DISK.txt', 'w') as f:
        ...
        print("Performance metrics have been saved to q3_performance_persist_MEMORY_AND_DISK.txt")
