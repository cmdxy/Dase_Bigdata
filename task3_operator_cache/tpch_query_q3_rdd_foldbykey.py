...
def main():
    # 初始化 SparkContext
    conf = SparkConf().setAppName("TPCH_Query3_RDD_Fold")
    ...
    try:
        ...
        # 使用 foldByKey 进行优化
        revenue_sum = revenues.foldByKey(
            0.0,  # 初始值
            lambda acc, value: acc + value  # 合并函数
        )
        ...
        # 将结果写入本地文件
        with open('q3_results_foldbykey.txt', 'w') as f:
            f.write(header)
            for row in all_results:
                f.write(row + '\n')

        print("Results have been saved to q3_results_foldbykey.txt")
        ...
        with open('q3_performance_foldbykey.txt', 'w') as f:
        ...
        print("Performance metrics have been saved to q3_performance_foldbykey.txt")
