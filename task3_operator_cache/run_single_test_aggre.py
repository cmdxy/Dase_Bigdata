...
def run_single_test(script_path):
    ...
    try:
        ...
        # 添加性能指标（如果存在）
        if os.path.exists('q3_performance_aggregatebykey.txt'):
            with open('q3_performance_aggregatebykey.txt', 'r') as f:
                result["performance_metrics"] = f.read()
        
        # 保存结果到JSON文件
        result_file = f"{output_dir}/single_test_result_aggregatebykey_{timestamp}.json"
...
def main():
    script_path = "tpch_query_q3_rdd_aggregatebykey.py"
