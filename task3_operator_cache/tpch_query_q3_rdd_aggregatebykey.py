...
def main():
    # 初始化 SparkContext
    conf = SparkConf().setAppName("TPCH_Query3_RDD_Aggregate")
    ...
    try:
        ...
        # 使用 aggregateByKey 进行优化
        revenue_sum = revenues.aggregateByKey(
            0.0,  # 初始值
            lambda acc, value: acc + value,  # 合并值到累加器
            lambda acc1, acc2: acc1 + acc2   # 合并两个累加器
        )
        ...
        # 将结果写入本地文件
        with open('q3_results_aggregatebykey.txt', 'w') as f:
            f.write(header)
            for row in all_results:
                f.write(row + '\n')

        print("Results have been saved to q3_results_aggregatebykey.txt")
        ...
        with open('q3_performance_aggregatebykey.txt', 'w') as f:
        ...
        print("Performance metrics have been saved to q3_performance_aggregatebykey.txt")
